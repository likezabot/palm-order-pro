-- Verificação de consistência: assinaturas únicas e esperadas das RPCs críticas.
-- Retorna lista de divergências; vazio = tudo OK.
...
DECLARE
  ...
  -- Catálogo esperado: nome -> array de assinaturas aceitáveis (oidvector via pg_get_function_identity_arguments)
  -- Usamos array_length=1 para "deve existir exatamente 1". Para funções com sobrecargas legítimas,
  -- usamos array_length>1 e validamos que TODAS as assinaturas listadas existem.
...
BEGIN
  ...
  LOOP
    -- 1) Conta quantas funções com este nome existem no schema public
    SELECT count(*)
      INTO v_count
    FROM pg_proc p
    ...
    -- 2) Para funções com 1 assinatura esperada: count deve ser 1
    ...
    -- 3) Verifica que cada assinatura esperada existe
    ...
    LOOP
      ...
      IF v_count = 0 THEN
        -- Coleta assinaturas REAIS para diagnóstico
  ...
  RETURN jsonb_build_object(
    'checked_at', now(),
    'total_functions_checked', jsonb_object_keys(expected) IS NOT NULL,
...
REVOKE ALL ON FUNCTION public.verify_rpc_consistency() FROM public;
GRANT EXECUTE ON FUNCTION public.verify_rpc_consistency() TO anon, authenticated, service_role;