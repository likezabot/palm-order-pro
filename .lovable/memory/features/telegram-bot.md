...
**Modo turbo (contexto da última mesa por chat):** `Map<chatId, {table, ts}>` in-memory, TTL 15min, best-effort (cold starts derrubam). `setLastTable` chamado SÓ em sucesso (executeAdd/Remove/View no `runExecute` via `successTable`, e no `handleCallbackQuery` após `executeAdd/Remove`). Parser produz `ADD_NOMESA`/`REMOVE_NOMESA`/`VIEW_NOMESA` quando linha tem operador (ou gatilho VIEW) sem `mesa N`. `resolveWithContext(cmd, chatId)` (chamado no `Deno.serve` antes de `handleCommand`) converte em `ADD/REMOVE/VIEW` com `fromContext: true` se houver mesa no contexto, senão devolve `NEEDS_TABLE` (mensagem `⚠️ Não sei qual mesa usar...`). Respostas com `fromContext` recebem prefixo `📍 (mesa N, contexto)\n`. Multi-comando: contexto é atualizado entre linhas, então `mesa 3 + 1 coca\\nmais um boi` funciona. Preview usa contexto via `previewCommand(cmd, chatId)` mas NUNCA grava (`(preview, mesa N do contexto)`); sem contexto retorna `⚠️ (preview) Nenhuma mesa em contexto.` HELP_TEXT inclui seção "💨 Atalhos".
