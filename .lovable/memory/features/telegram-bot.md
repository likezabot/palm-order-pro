...
**Modo turbo (contexto da última mesa por chat):** contexto agora é persistido em `settings` por chat (`key = telegram_last_table:<chat_id>`) com payload JSON `{ table, ts }` e TTL 15min, para sobreviver a cold starts da edge function. `getLastTable(chatId)` e `setLastTable(chatId, table)` são assíncronos e leem/escrevem na tabela `settings`. `setLastTable` roda só em sucesso real: ADD quando resposta começa com `✅ Mesa`, REMOVE quando começa com `➖ Mesa` ou `⚠️ Removidos`, VIEW quando a resposta não é `⚠️ Mesa N não tem pedido aberto.`; também roda no `handleCallbackQuery` após sucesso real do botão inline. `resolveWithContext` virou async e é chamado com `await` no fluxo single/multi-comando; preview também usa `await getLastTable(chatId)` mas nunca grava. Respostas com contexto seguem com prefixo `📍 (mesa N, contexto)`.
